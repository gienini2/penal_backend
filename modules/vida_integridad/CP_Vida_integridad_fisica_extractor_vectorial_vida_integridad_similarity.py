...
def cargar_catalogo(path_base):
    ...
    for file in os.listdir(path_base):
        if file.startswith("CP-Vida") and file.endswith(".json"):
            nombre = file.replace("CP-Vida-","").replace(".json","")
            with open(os.path.join(path_base,file), "r", encoding="utf-8") as f:
                matriz[nombre] = vector_to_list(json.load(f))
    return matriz

def comparar_vida_integridad(vector_texto, path_base, umbral=0.5):
